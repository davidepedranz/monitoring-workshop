...
class PostgresRepository(Repository):
    ...
    def factory(max_connections: int = 10):
        connection_url = os.environ.get("POSTGRES_CONNECTION_URL")
        if connection_url is None:
            raise AttributeError("Please specify a valid POSTGRES_CONNECTION_URL for Postgres in the environment")
        return PostgresRepository(connection_url=connection_url, max_connections=max_connections)
    ...
    def connect(self) -> None:
        ...
        self._pool = ThreadedConnectionPool(minconn=1, maxconn=self._max_connections, dsn=self._connection_url)
    ...
    @random_delay(min_delay=0.8, max_delay=1.2)
